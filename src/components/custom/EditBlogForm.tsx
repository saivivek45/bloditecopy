...
const BlogUpdateForm = ({ id }: { id: string }) => {
  ...
  const loadBlog = async () => {
    try {
      ...
      const blogData = response?.data?.data as Blog
      setData(blogData)
      if (blogData) {
        form.reset({
      ...
      const errorMessage = axiosError?.response?.data.message || "Failed to load blog"
  ...
  const onSubmit = async (formData: z.infer<typeof formSchema>): Promise<void> => {
    setIsSubmitting(true)
    try {
      const response = await axios.put(`/api/blog/updateBlogById`, {
        id: id, 
      ...
      const axiosError = error as AxiosError<ApiResponse>
      const errorMessage = axiosError?.response?.data.message || "Failed to update blog"
      toast.error(errorMessage)
    } 
    finally {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 flex items-center justify-center">
                ...
                <div className="relative flex items-center gap-3">
                  {isSubmitting ? (
                    <>
                      ...
                      <span className="animate-pulse">Updating Article...</span>
                    </>
                  ) : (
                    <>
